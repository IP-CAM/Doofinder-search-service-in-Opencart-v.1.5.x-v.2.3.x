<?php
class ControllerFeedDoofinderfeed extends Controller {
        private $error = array();
...
*/
    ...
    public function index() {
        $this->load->language('feed/doofinder_feed');
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('feed/doofinder_feed', 'token=' . $this->session->data['token'], 'SSL')
        );

        $data['action'] = $this->url->link('feed/doofinder_feed', 'token=' . $this->session->data['token'], 'SSL');
        ...
        if(isset($this->request->post['doofinder_feed_image_size'])){
            $data['doofinder_feed_image_size'] = $this->request->post['doofinder_feed_image_size'];
        } else{
            $data['doofinder_feed_image_size'] = $this->config->get('doofinder_feed_image_size') !== null ?
        ...
        if (isset($this->request->post['doofinder_feed_mpn_field'])){
            $data['doofinder_feed_mpn_field'] = $this->request->post['doofinder_feed_mpn_field'];
        } else {
            $data['doofinder_feed_mpn_field'] = $this->config->get('doofinder_feed_mpn_field') ? $this->config->get('doofinder_feed_mpn_field') : 'mpn';
        }

        // DISPLAY PRICES?
        if(isset($this->request->post['doofinder_feed_display_prices'])){
            $data['doofinder_feed_display_prices'] = $this->request->post['doofinder_feed_display_prices'];
        } else{
            $data['doofinder_feed_display_prices'] = $this->config->get('doofinder_feed_display_prices') !== null ? $this->config->get('doofinder_feed_display_prices') : 1;
        }

        // DISPLAY PRICES WITH TAXES?
        if(isset($this->request->post['doofinder_feed_prices_with_taxes'])){
            $data['doofinder_feed_prices_with_taxes'] = $this->request->post['doofinder_feed_prices_with_taxes'];
        } else{
            $data['doofinder_feed_prices_with_taxes'] = $this->config->get('doofinder_feed_prices_with_taxes') !== null ? $this->config->get('doofinder_feed_prices_with_taxes') : 1;
        }

        // USE FULL CATEGORY PATH
        if(isset($this->request->post['doofinder_feed_full_category_path'])){
            $data['doofinder_feed_full_category_path'] = $this->request->post['doofinder_feed_full_category_path'];
        } else{
            $data['doofinder_feed_full_category_path'] = $this->config->get('doofinder_feed_full_category_path') !== null ?
        ...
        echo "WTF";
        $this->response->setOutput($this->load->view('feed/doofinder_feed.tpl', $data));

    }

        protected function validate() {
                if (!$this->user->hasPermission('modify', 'feed/doofinder_feed')) {
